...
// Interface Output 
void print_ui(int Elapsed, int Total, int Volume, int Ispaused, bool OutputName, char* NameFile, bool islooping) {
...
int main(int argc, char *argv[]) {
    ...
    while (running && (Mix_PlayingMusic() || is_paused)) {
        ...
        print_ui(elapsed, total_time, (volume * 100) / MIX_MAX_VOLUME, is_paused, soutputname, filename, isloop);
	    ...
	    case kquit:
            ...
            case kshortback:
                if (Mix_GetMusicPosition(music) >= sshortstep) {
                    if (Mix_SetMusicPosition(Mix_GetMusicPosition(music) - sshortstep) == -1) {
        ...
        // Если музыка закончилась и не в режиме loop, выходим
    ...
    Mix_HaltMusic();           // Останавливаем музыку
    Mix_FreeMusic(music);      // Освобождаем музыку
